...
def train_loop_parallel(config, model, criterion, train_loader, test_loader):
    """使用定点迭代和滑动窗口的并行训练循环"""
    # 如果只有一个GPU，退回到常规训练

    if config.device_count <= 1:
        train_loop_serial(config, model, criterion, train_loader, test_loader)
        return
    ...
    for rank in range(1, config.device_count):
        ...
        worker_optimizer = optim.SGD(worker_model.parameters(), 
                                    lr=config.learning_rate, 
                                    momentum=config.momentum)
                                    
        p = mp.Process(target=run_worker, 
                      args=(worker_model, worker_optimizer, criterion, 
...
def main():
    ...
    # 使用并行训练循环
    trained_model = train_loop_parallel(config, model, criterion, train_loader, test_loader)
